...
public class PlaceOrderScreen {
    ...
    void showInvoiceScreen() {
        ...
        float deliveryFee = 1 + random.nextFloat() * (5 - 1);
    ...
    public static String generateTransactionId(int length) {
        ...
        for (int i = 0; i < length; i++) {
            int[] ranges = { 48, 65, 97 }; // ASCII values for '0', 'A', 'a'
            int[] rangeLengths = { 10, 26, 26 }; // Number of characters in each range

            int rangeIndex = rd.nextInt(3); // Randomly select which range to use
            int charCode = ranges[rangeIndex] + rd.nextInt(rangeLengths[rangeIndex]);
